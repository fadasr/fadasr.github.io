...
RSAES-OAEP can operate on messages of length up to `k - 2hLen - 2 octets`, where `hLen` is the length of the output from the underlying hash function and `k` is the length in octets of the recipient's RSA modulus. With RSA 1024-bit 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNzAzNzAyNzIsNTUyNjA3NTQ5LC04NT
