...
the size of the message that can be encrypted using RSA is limited by the size of `n`. With encoding function (e.g. [RSAES-OAEP](https://tools.ietf.org/html/rfc8017#page-18)) we cannot even use all the bits.
...
the RSAEP primitive performs the public key RSA transform by raising the integer to `e` modulo `n`. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA1MzQ1MDIwMiwxNDYyNDY2ODUyLDg4MT
