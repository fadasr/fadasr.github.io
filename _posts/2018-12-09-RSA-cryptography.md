...
RSAES-OAEP can operate on messages of length up to `k - 2hLen - 2` octets, where `hLen` is the length of the output from the underlying hash function and `k` is the length in octets of the recipient's RSA modulus. With RSA 2048-bit modulus, the message limit for OAEP can be calculated:
```
SHA-256: 
...
for hybrid mode, the typical largest message would be 32 bytes corresponding to a 256-bit AES key.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2Nzc0OTM5NDUsNTUyNjA3NTQ5LC04NT
