...
- one-way function: a function that can be computed but cannot be inverted. aka pre-image attack resistance: Finding the message `m` given only the digest `h` is nontrivial
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgzMzE4MDIxMSwyMDE5NDc4MTMyLC0xMz
